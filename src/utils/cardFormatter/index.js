...
export const cardSerialNumberFormatter = (serialnumber) => {
    ...
    if (serialnumber.replace(/\W/gi, '').length > 16) return serialnumber.substring(0, 19);
    const { card } = cardValidator.number(serialnumber);
    console.log(serialnumber.match(/[0-9●]{1,3}/));
    if (card?.type === 'american-express') {
        const fir = serialnumber.replace(/\W/gi, '').match(/[0-9●]{1,10}/)[0]?.substring(0, 4)
        const sec = serialnumber.replace(/\W/gi, '').match(/[0-9●]{1,10}/)[0]?.substring(4, 10);
        const thir = serialnumber.replace(/\W/gi, '').match(/[0-9●]{1,15}/)[0]?.substring(10, 15);
    ...
    return serialnumber.match(/[0-9●]{1,4}/g)?.join(' ').substring(0, 19) || serialnumber.substring(0, 19);
};
export function expirationDateFormatter(
    serialnumber,
) {
    if (!serialnumber) return;
    if (serialnumber.replace(/\W/gi, '').length > 4) return serialnumber.substring(0, 5);
    return serialnumber.match(/[0-9●]{1,2}/g)?.join('/').substring(0, 5) || serialnumber.substring(0, 5);
}
