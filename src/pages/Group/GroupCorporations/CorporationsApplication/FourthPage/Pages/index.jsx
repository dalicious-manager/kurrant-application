...
import { corpApplicationSpotsAtom, corpApplicationTotalSpotAtom, corpApplicationUseMealType, isCorpFullSpotAddressAtom, isCorpMealInfoAtom, isCorpSendSpotAddressInfoAtom } from "../../../../../../biz/useCorporationApplication/store";
...
const Pages = () => {
    ...
    const isValidation = 
        (spotNameChk && !errors.spotName) &&
        (addressChk &&!errors.address) &&
            ...
            <FormProvider {...form}>
                <KeyDismiss onPress={()=>Keyboard.dismiss()}>
                    <Container>
                        ...
                        placeholder="배송지"
                        style={inputStyle}
                        onPressIn={()=>navigation.navigate(CorporationApplicationSpotPostCodePageName)}
                    ...
                    {isMealInfo.map((m,idx) => {
                      ...
                      return (
                        <Meal key={idx} touch={touch?.includes(idx+1)} onPress={() => onPressButton(idx+1)}>
...
border-color:${({theme,touch}) => !touch ? theme.colors.grey[8] : theme.colors.grey[7]};
border-width:1px;
border-radius:14px;
