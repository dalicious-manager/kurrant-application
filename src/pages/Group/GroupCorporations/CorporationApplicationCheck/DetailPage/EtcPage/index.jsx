...
export const PAGE_NAME = "P__GROUP__CREATE__CORPORATION__APPLICATION__ETC" ;
const Pages = ({route}) => {
    const id = route.params.data.id;
    
    const navigation = useNavigation();
    
    const {corporationApplicationMemo,setCorpCheck,isCorpCheck} = useCorporationApplication();
    
    const [text,setText] = useState('');
    
    const additionPress = async () => {
        try {
           await corporationApplicationMemo({
            'memo' : text,

           },id);

           await setCorpCheck((prevState) => ({
            ...prevState,
            option: {
                ...prevState.option,
                memo:text
    ...
    return (
        <SafeArea>
            <Wrap>
                ...
                defaultValue={isCorpCheck.option.memo}
