...
const Pages = () => {
  ...
  const {apartApplicationCheck, isApartCheck, isApartLoading} =
    useApartApplication();
    ...
    <Wrapper>
      <ScrollViewWrap>
        ...
        <ContentsWrap>
          ...
          <ProgressAllWrap>
            <ProgressBarWrap>
              {((state === 0 && isApartCheck.progressStatus === 0) ||
                (state === 1 && isCorpCheck.progressStatus === 0)) && (
                <ProgressBar progress={1} />
              )}
              {((state === 0 && isApartCheck.progressStatus === 1) ||
                (state === 1 && isCorpCheck.progressStatus === 1)) && (
                <ProgressBar progress={2} />
              )}
              {((state === 0 && isApartCheck.progressStatus === 2) ||
                (state === 1 && isCorpCheck.progressStatus === 2)) && (
                <ProgressBar progress={3} />
              )}
              {((state === 0 && isApartCheck.progressStatus === 3) ||
                (state === 1 && isCorpCheck.progressStatus === 3)) && (
            ...
            <ProgressTitleWrap>
              ...
              {((state === 0 && isApartCheck.progressStatus !== 3) ||
                (state === 1 && isCorpCheck.progressStatus !== 3)) && (
                <ProgressTitle>스팟 개설 완료</ProgressTitle>
              )}
              {((state === 0 && isApartCheck.progressStatus === 3) ||
                (state === 1 && isCorpCheck.progressStatus === 3)) && (
                <>
                  ...
                  <RejectWrap>
                    <TextButton
                      ...
                      onPressEvent={() =>
                        state === 0
                          ? navigation.navigate(
                              ApartmentApplicationRejectPageName,
                              {reason: isApartCheck.rejectedReason},
                            )
                          : navigation.navigate(
                              ApartmentApplicationRejectPageName,
                              {reason: isCorpCheck.rejectedReason},
                            )
                      }
                    />
                  </RejectWrap>
        ...
        <ContentsWrap>
          <HeadingWrap>
            ...
            <DetailWrap>
              <TextButton
                ...
                onPressEvent={() => {
                  state === 0
                    ? navigation.navigate(ApartmentApplicationDetailPageName, {
                        data: isApartCheck,
                        id: propsId,
                      })
                    : navigation.navigate(
                        CorporationApplicationDetailPageName,
                        {data: isCorpCheck, id: propsCorpId},
                      );
                }}
              />
              <ArrowRightIcon />
          ...
          <TitleWrap>
            <Title>
              {state === 0 ? '서비스 이용 단체명' : '서비스 이용사명'}
            </Title>
            <TitleContent>
              {state === 0 ? `${name}` : `${corpName}`}
            </TitleContent>
          </TitleWrap>
          <TitleWrap>
            <Title>서비스 신청자명</Title>
            <TitleContent>
              {state === 0 ? `${user?.name}` : `${corpUser?.name}`}
            </TitleContent>
          </TitleWrap>
          <TitleWrap>
            <Title>신청자 연락처</Title>
            <TitleContent>
              {state === 0 ? `${user?.phone}` : `${corpUser?.phone}`}
            </TitleContent>
          </TitleWrap>
          <TitleWrap>
            <Title>신청자 이메일</Title>
            <TitleContent>
              {state === 0 ? `${user?.email}` : `${corpUser?.email}`}
            </TitleContent>
          </TitleWrap>
        </ContentsWrap>
      </ScrollViewWrap>
