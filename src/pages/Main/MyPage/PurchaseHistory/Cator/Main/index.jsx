...
import { purchaseCatorAtom } from "../../../../../../biz/usePurchaseHistory/store";
...
const Pages = () => {
  ...
  const form = useForm();
  const [,setCatorPurchase] = useAtom(purchaseCatorAtom);
...
const confirmPress = (setModal) =>{    
  ...
  const {getPurchaseHistory, readAbleAtom:{catorPurchase ,isCatorPurchaseLoading}} = usePurchaseHistory();
  ...
  return (
    ...
    <Container>
      <Wrapper>
        ...
        {isCatorPurchaseLoading ? <Skeleton />:
        <Wrapper>
        {catorPurchase?.data ? <ScrollViewBox>{catorPurchase?.data?.map((v,i)=>{
