...
const Pages =  ({route})=>{
    ...
    return(
        <Wrapper paddingTop={13}>
            <TitleBox>
                <Typography text={'Title02SB'} textColor={themeApp.colors.grey[2]}>{list[0].title}</Typography>
            </TitleBox>
            {list[0].question && list.map((v)=>{
                return( <ListBox key={v.id} title={v.question} onPressEvent={()=>{
