...
const Component = ({size = 40, color, social='NAVER', isConnect=false}) => {
  ...
  const SocialIcon = (socialName)=>{
    switch (socialName) {      
      ...
      case 'KAKAO':
        if(isConnect){
          return  <KakaoConnectIcon
          size={size}
          color={color}
        />;
        }
        return  <KakaoDisConnectIcon 
          size={size}
          color={color}
        />;
      ...
      case 'FACEBOOK':
        if(isConnect){
          return  <FacebookConnectIcon
          size={size}
          color={color}
        />;
        }
        return <FacebookDisConnectIcon 
          size={size}
          color={color}
        />;
      case 'GOOGLE':
        if(isConnect){
          return  <GoogleConnectIcon
          size={size}
          color={color}
        />;
        }
        return <GoogleDisConnectIcon 
          size={size}
          color={color}
        />;
      case 'NAVER':
        if(isConnect){
          return  <NaverConnectIcon
          size={size}
          color={color}
        />;
        }
        return <NaverDisConnectIcon 
          size={size}
          color={color}
        />;
      default:
