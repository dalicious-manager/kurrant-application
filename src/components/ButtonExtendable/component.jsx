    ...
    <Container>
      <BtnContainer>
        ...
        <InnerContainer
            ...
            <AnimationView
              ...
              <Inner>
                ...
                <CountView>
                  ...
                  />
                  <CapacityText
                    capacity={Number(capacity) - Number(cartCount)}
                    count={Number(count)}>
                    재고수량 : {Number(capacity) - Number(cartCount || 0)}
                  </CapacityText>
                </CountView>
              </Inner>
            </AnimationView>
          ...
          <PressButton
            onPress={onPressEvent2}
            capacity={Number(capacity) - Number(cartCount || 0)}
            count={Number(count)}
            disabled={
              Number(capacity) < Number(count) + Number(cartCount || 0)
            }>
            <ButtonText
              capacity={Number(capacity) - Number(cartCount || 0)}
              count={Number(count)}>
...
const PressButton = styled.Pressable`
  background-color: ${({theme, capacity, count}) =>
    count > capacity ? theme.colors.grey[7] : theme.colors.yellow[500]};
  width: 100%;
  height: 56px;
  border-radius: 29px;
  padding: 16px;
...
const ButtonText = styled(Typography).attrs({text: 'BottomButtonSB'})`
  color: ${({theme, capacity, count}) =>
    count > capacity ? theme.colors.grey[0] : theme.colors.grey[2]};
  text-align: center;
`;

const CapacityText = styled(Typography).attrs({text: 'CaptionR'})`
  color: ${({theme, capacity, count}) =>
    count > capacity ? theme.colors.red[500] : theme.colors.grey[2]};
`;
