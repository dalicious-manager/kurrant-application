      ...
      <Wrap
        ...
        {btn.map((week, idx) => (
          <Btn
            ...
            chk={chk}>
            <WeekText
              text={chk === idx ? 'Button10SB' : 'Button10R'}
              checked={chk}
              idx={idx}>
              {week}
            </WeekText>
...
const Btn = styled.Pressable`
  ...
  border: 1px solid;
  border-radius: 7px;
  border-color: ${({checked, idx, chk}) =>
    checked === idx
      ? props => props.theme.colors.grey[2]
      : props => props.theme.colors.grey[8]};
  background-color: ${({checked, idx, chk}) =>
    checked === idx
      ? props => props.theme.colors.grey[0]
      : props => props.theme.colors.grey[8]};
  /* align-self:flex-start; */
  ${({idx}) =>
    idx === 0 ? 'margin-left:28px' : idx === 3 && 'margin-right:28px'}
`;

const WeekText = styled(Typography)`
  color: ${({checked, idx, chk}) =>
    checked === idx
      ? props => props.theme.colors.grey[2]
      : props => props.theme.colors.grey[5]};
`;
