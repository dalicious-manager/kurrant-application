...
const Component = () => {
  ...
  let uploadFileLabel = fileResponse[0] ? fileResponse[0]?.name.slice(-25) + (fileResponse.length > 1 ? ` 외 ${fileResponse.length - 1}개` : '') : '선택된 파일 없음';
  let checkedVolume = fileResponse.map(item => item.size).reduce((prevVolume, currentVolume) => prevVolume + currentVolume, 0) > (10 * 1024 * 1024);
